...
- (BOOL)start3DMouse
{
	OSErr	error = -1; // something but != noErr
  NSLog(@"start3DMouse");
...
void messageHandler3DMouse(io_connect_t connection, natural_t messageType, void *messageArgument)
{
  ...
  NSLog(@"ConnexionDeviceStatesizeof..  %ld", sizeof(ConnexionDeviceState));
  NSLog(@"ConnexionDevicePrefs sizeof..  %ld", sizeof(ConnexionDevicePrefs));
	...
	switch(messageType)
	{
        ...
        case kConnexionCmdHandleAxis:
          {
            ...
            NSLog(@"axis sizeof..  %ld", sizeof(state->axis));
      ...
      NSLog(@"PrefsChanged - %x VendorID: %x, DeviceID: %x", vidPid, (vidPid & 0xFF00), (vidPid & 0x00FF));
        ...
        NSLog(@"PrefsChanged - 3D Mouse prefs for device 0x%x and application %@", kDevID_SpaceMousePro, appName);   
			}
      break;
		default:
